...
[ "true" = "${INSTALL_LIB}" ] \
  && log "devcontainer-install" "installing install-lib.sh" \
...
if [ "true" = "${INSTALL_FZF}" ] || [ "true" = "${GIT_PROMPT}" ]; then
  log "devcontainer-install" "installing cURL"
  install_packages curl ca-certificates
  log "devcontainer-install" "creating download dir"
...
[ "true" = "${ENSURE_BASH}" ] && [ ! $(command -v bash) ] \
  && log "devcontainer-install" "installing bash" \
...
if [ -n "${BASH_HISTORY_PATH}" ]; then
  if [ -n "${HISTORY_PATH:="${BASH_HISTORY_PATH%\.bash_history}"}" ]; then
    ...
    log "devcontainer-install" "setting bash history location to ${HISTORY_PATH}"
    mkdir -p "${HISTORY_PATH}" && touch "${HISTORY_PATH}/.bash_history" 
    chmod -R ugo+rw "${HISTORY_PATH}"

    log "devcontainer-install" "setting user bash history location(s)"
    SNIPPET="export HISTFILE=${HISTORY_PATH}.bash_history"
    run_command_for_users "echo \"${SNIPPET}\" >> ~/.bashrc"
  fi
fi
...
if [ "true" = "${INSTALL_FZF}" ]; then
  ...
  log "devcontainer-install" "downloading fzf from ${URL}"
  curl -sfL --retry 3 "${URL}" --output "${DOWNLOAD_DIR}/install-fzf"
  install_script "${DOWNLOAD_DIR}/install-fzf" /opt/jjs105/bin

  log "devcontainer-install" "installing fzf for users"
  run_command_for_users "/opt/jjs105/bin/install-fzf --all"
fi
...
if [ "true" = "${GIT_PROMPT}" ]; then
  ...
  log "devcontainer-install" "downloading git-prompt from ${URL}"
  curl -sfL --retry 3 "${URL}" --output "${DOWNLOAD_DIR}/git-prompt.sh"
  install_library "${DOWNLOAD_DIR}/git-prompt.sh" /opt/jjs105/lib

  log "devcontainer-install" "configuring git prompt for users"
...
[ -n "${DOWNLOAD_DIR}" ] \
  && log "devcontainer-install" "removing download dir" \
  && rm -r "${DOWNLOAD_DIR}"
